  ...
  return state.clientes
    ...
    .filter((c) => {
      ...
      const domiciliosText = domiciliosCliente(c)
        ...
        .join(" ");
      return [c.nombre, c.telefono, c.correo, c.direccion, c.tipo, c.ciudad, c.observaciones, domiciliosText]
...
function exportClientesCsv() {
  ...
  downloadCsv(
    ...
    ["Cliente", "Telefono", "Correo", "Ciudad", "Tipo", "Direccion principal", "Domicilios", "Observaciones", "Servicios", "Facturado", "Cobrado", "Por cobrar"],
    rows
  );
}
...
function formDomiciliosCliente(data) {
  ...
  const rows = Array.from({ length: 5 }, (_, index) => domicilios[index] || {});
  return `
    <div class="full panel">
      <h2>Domicilios del cliente</h2>
      <div class="form-grid">
        ${rows.map((domicilio, index) => `
          ${input(`domicilioAlias${index}`, `Alias domicilio ${index + 1}`, domicilio.alias || (index === 0 ? "Principal" : ""))}
          ${select(`domicilioCiudad${index}`, "Ciudad", domicilio.ciudad || data.ciudad || "MERIDA", ["MERIDA", "CDMX"].map((x) => ({ value: x, label: x })))}
          ${input(`domicilioDireccion${index}`, "Direccion", domicilio.direccion || "", "text", "wide")}
...
function formFor(type, data) {
  if (type === "cliente") {
    return `<div class="form-grid">${input("nombre", "Cliente", data.nombre, "text", "wide")}${select("ciudad", "Ciudad principal", data.ciudad || "MERIDA", ["MERIDA", "CDMX"].map((x) => ({ value: x, label: x })))}${input("telefono", "Telefono", data.telefono)}${input("correo", "Correo / contacto", data.correo)}${select("tipo", "Tipo", data.tipo, ["Residencial", "Comercial", "Industrial", "Gobierno", "Otro"].map((x) => ({ value: x, label: x })))}${text("observaciones", "Observaciones", data.observaciones, "full")}${formDomiciliosCliente(data)}</div>`;
  }
  if (type === "producto") {
