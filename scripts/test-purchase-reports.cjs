...
run(`state.compras[0].pagadoPor='VICTOR'; state.compras[1].pagadoPor='SISPROVISA'; state.compras[2].pagadoPor='VICTOR'; state.compras[3].pagadoPor='VICTOR'; state.equipos[0].pagadoPor='VICTOR'; state.equipos[1].pagadoPor='SISPROVISA';`);
assert.equal(run(`acquisitionReportData('compras',range,'','VICTOR').total`),20);
assert.equal(run(`acquisitionReportData('compras',range,'','SISPROVISA').total`),30);
assert.equal(run(`acquisitionReportData('compras',range,'','Sin dato').total`),0);
assert.equal(run(`acquisitionReportData('compras',null,'','Sin dato').total`),7);
assert.equal(run(`acquisitionReportData('compras',range,'SISPROVISA','VICTOR').total`),0);
...
assert.match(html, /Pagado por:<\/strong> VICTOR/);
assert.doesNotMatch(html, /SISPROVISA/);
...
const handlers = {};
const elements = { mode: {value:'month',addEventListener() {}}, start:{value:'2024-02'}, search:{value:'Bomba'}, payer:{value:'VICTOR'} };
...
assert.equal(run('dispatch[3]'), 'VICTOR');
