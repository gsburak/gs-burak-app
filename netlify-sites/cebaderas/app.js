...
function validate(data){
  ...
  for(const key of keys){
    if(typeof data[key]!=='string' || data[key].length>(key==='notas'?2000:500)) throw new Error('El archivo contiene datos inválidos o demasiado largos.');
    result[key]=data[key];
  }
  result.estaciones=data.estaciones.map(row=>Object.fromEntries(columns.map(key=>{
    if(!row || typeof row[key]!=='string' || row[key].length>limits[key]) throw new Error('Una estación contiene datos inválidos o demasiado largos.');
    return [key,row[key]];
  })));
  return result;
}
...
function renderRows(){
  ...
  $('#rows').innerHTML=report.estaciones.map((row,i)=>`<tr>${columns.map(key=>`<td>${key==='numero'||key==='condiciones'?`<input data-row="${i}" data-key="${key}" aria-label="${key} de estación ${i+1}" maxlength="${limits[key]}" ${key==='condiciones'?'list="condiciones"':''} value="${escapeHtml(row[key])}">`:`<textarea data-row="${i}" data-key="${key}" aria-label="${key} de estación ${i+1}" maxlength="${limits[key]}" rows="2">${escapeHtml(row[key])}</textarea>`}</td>`).join('')}<td><button type="button" class="remove" data-remove="${i}" aria-label="Eliminar estación ${i+1}" ${report.estaciones.length===1?'disabled':''}>×</button></td></tr>`).join('');
}
...
function meta(label,value){return `<div><b>${label}</b>${escapeHtml(value||'—')}</div>`;}
function renderPreview(){
  ...
  function page(first=false){
    ...
    sheet.innerHTML=`<div class="paper-content"><header class="report-head"><div><h2>Inspección y servicio</h2><p>ESTACIONES CEBADERAS · GS BURAK</p><p>${escapeHtml(report.cliente||'Cliente por completar')} · Folio ${escapeHtml(report.folio||'—')}</p></div><img src="assets/gs-burak-logo.jpeg" alt="GS Burak"></header>${first?`<section class="client-details">${meta('Cliente',report.cliente)}${meta('Fecha / hora',`${report.fecha} ${report.hora}`.trim())}${meta('Domicilio',report.domicilio)}${meta('Ciudad',report.ciudad)}${meta('Correo',report.correo)}${meta('Teléfono',report.telefono)}</section>`:''}</div><footer class="paper-footer"><span>GS BURAK · ${report.estaciones.length} estaciones</span><span class="page-number"></span></footer>`;
    preview.append(sheet);pages.push(sheet);return sheet.querySelector('.paper-content');
  }
  function table(content){
    const node=document.createElement('table');node.className='report-table';node.innerHTML='<thead><tr><th>N.º</th><th>Ubicación</th><th>Observaciones</th><th>Condiciones</th><th>Acción</th></tr></thead><tbody></tbody>';content.append(node);return node.querySelector('tbody');
  }
  ...
  for(const station of report.estaciones){
    const tr=document.createElement('tr');tr.innerHTML=columns.map(key=>`<td>${escapeHtml(station[key]||'—')}</td>`).join('');body.append(tr);
  ...
  closing.innerHTML=`<h3>OBSERVACIONES GENERALES</h3><p>${escapeHtml(report.notas||'Sin observaciones adicionales.')}</p><div class="signatures">${[['Responsable sanitario',report.responsable],['Técnico',report.tecnico],['Cliente / recibe',report.recibe]].map(([role,name])=>`<div>${escapeHtml(name||'Nombre y firma')}<small>${role}</small></div>`).join('')}</div>`;
  ...
  $('#summary').textContent=`${report.estaciones.length} estaciones · ${pages.length} ${pages.length===1?'página':'páginas'}`;
...
$('#rows').addEventListener('click',event=>{
  ...
  if(columns.slice(1).some(k=>row[k].trim())&&!confirm(`¿Eliminar la estación ${row.numero}? Se quitarán los datos de esta fila.`))return;
...
function resize(count){
  ...
  if(count<report.estaciones.length&&report.estaciones.slice(count).some(r=>columns.slice(1).some(k=>r[k].trim()))&&!confirm(`Al reducir a ${count} estaciones se eliminarán las últimas ${report.estaciones.length-count} filas y sus datos. ¿Continuar?`)){$('#count').value=report.estaciones.length;return;}
  ...
  renderRows();changed();status(`${count} estaciones. La vista previa se ajusta automáticamente.`);
}
...
$('#visit').addEventListener('click',()=>{
  ...
  dirty=true;draft();showReport();status('Nueva visita: se conservaron el cliente y las ubicaciones; captura los resultados de esta inspección.');
});
...
$('#import-file').addEventListener('change',async event=>{
  ...
  try{if(file.size>2500000)throw new Error('El archivo es demasiado grande.');const next=validate(JSON.parse(await file.text()));if(!canReplace())return;report=next;dirty=true;draft();showReport();status('Editable abierto. Guarda el reporte para añadirlo a este navegador.');}
...
$('#catalog').addEventListener('change',()=>{
  ...
  if((report.cliente||report.estaciones.some(r=>columns.slice(1).some(k=>r[k])))&&!confirm('Cambiar de cliente inicia un reporte con estaciones vacías. Guarda el actual si deseas conservarlo. ¿Continuar?')){$('#catalog').value=report.clienteId;return;}
  report=newReport();report.clienteId=String(client.id);report.cliente=client.nombre||'';report.domicilio=client.direccion||'';report.telefono=client.telefono||'';report.correo=client.correo||'';showReport();changed();status('Cliente seleccionado. Indica cuántas estaciones tiene.');
});
...
try{const raw=localStorage.getItem(DRAFT);if(raw){report=validate(JSON.parse(raw));dirty=true;status('Se recuperó el último borrador de este navegador.');}}catch(_){status('No se pudo recuperar el borrador anterior. Puedes abrir un archivo editable.',true);}
