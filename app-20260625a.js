...
function programacionClienteFields(value) {
  ...
  return `
    <div class="field wide">
      <label>Buscar cliente</label>
      <input id="programacionClienteSearch" type="search" placeholder="Escribe nombre, telefono o domicilio" autocomplete="off" />
    </div>
    <div class="field wide">
      <label>Cliente</label>
      <select id="programacionClienteSelect" name="clienteId" required>
        <option value="">Selecciona un cliente</option>
        ${clientes.map((cliente) => `<option value="${cliente.id}" ${String(cliente.id) === String(value) ? "selected" : ""}>${cliente.nombre}</option>`).join("")}
      </select>
      <small id="programacionClienteCount" class="readonly">${clientes.length} clientes disponibles</small>
    </div>
  `;
}
...
function bindProgramacionClienteSearch() {
  ...
  const clientSelect = document.querySelector("#programacionClienteSelect");
  const countLabel = document.querySelector("#programacionClienteCount");
  if (!searchInput || !clientSelect) return;

  let selectedValue = clientSelect.value;
  const clientes = [...state.clientes]
    .sort((a, b) => String(a.nombre || "").localeCompare(String(b.nombre || "")));
  ...
  const updateOptions = () => {
    ...
    const filtered = term ? clientes.filter((cliente) => matchesSearch(cliente, term)) : clientes;
    clientSelect.innerHTML = `
      <option value="">${filtered.length ? "Selecciona un cliente" : "No se encontraron clientes"}</option>
      ${filtered.map((cliente) => `<option value="${cliente.id}">${cliente.nombre}</option>`).join("")}
    `;
    if (filtered.some((cliente) => String(cliente.id) === String(selectedValue))) {
  ...
  clientSelect.addEventListener("change", () => {
    selectedValue = clientSelect.value;
  });
}
